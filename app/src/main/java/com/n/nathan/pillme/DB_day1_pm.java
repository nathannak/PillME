...
public class DB_day1_pm extends SQLiteOpenHelper  {

    private static final int DATABASE_VERSION = 1;
    private static final String DATABASE_NAME = "med_1_pmDB.db";
    private static final String TABLE_MED_1_PM = "med_1_pm";
    ...
    public DB_day1_pm(Context context, SQLiteDatabase.CursorFactory factory)
    {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_TABLE_MED_1_PM = "CREATE TABLE IF NOT EXISTS " +
                TABLE_MED_1_PM + "("
                + COLUMN_SYMBOL +
                " TEXT , "
                + COLUMN_NAME +
        ...
        db.execSQL(CREATE_TABLE_MED_1_PM);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion,
                          int newVersion) {
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_MED_1_PM);
        onCreate(db);
    }
    ...
    public void addMeds() {
        ...
        //
        SharedPreferences pref  = ctx.getSharedPreferences("day1-pm",Context.MODE_PRIVATE);
        //
        ...
        for( int i=1 ; i<6 ; i++ ) {
            ...
            Log.v("DB_day1_pm", "saved med" + i + " symbol " + pref.getString("med"+i+"-symbol", null) );
            ...
            Log.v("DB_day1_pm", "minute value for med" + i + " is: " + Integer.toString(pref.getInt("med"+i+"-minute", 0) ));

            values.put(COLUMN_HASALARMIMAGE, Boolean.toString(pref.getBoolean("med"+i+"-alarm",false)) );
            Log.v("DB_day1_pm", "boolean value for saved alarm:" + Boolean.toString(pref.getBoolean("med"+i+"-alarm",false))  , null );
            ...
            db.insert(TABLE_MED_1_PM, null, values);
    ...
    {
        //
        //String query = "Select * FROM " + TABLE_MED_1_PM + " WHERE " + COLUMN_ID + " =  \"" + id + "\" ORDER BY " + COLUMN_ORDER + " DESC";
        String query = "Select * FROM " + TABLE_MED_1_PM;

        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(query, null);

        int cnt = cursor.getCount();
        Log.v("DB_day1_pm", " number of records in day1-pm DB: " + Integer.toString(cnt) );
        //
        SharedPreferences.Editor pref = ctx.getSharedPreferences("day1-pm",Context.MODE_PRIVATE).edit();
        ...
        Log.v("DB_day1_pm", " loaded med1-symbol " + cursor.getString(0) );
        //
        ...
        Log.v("DB_day1_pm", "loaded minute for med1" + " is: " + Integer.parseInt(cursor.getString(3)));
        //
        pref.putBoolean("med1-alarm",Boolean.parseBoolean(cursor.getString(4) ));
        Log.v("DB_day1_pm", "  Boolean value for loaded med1-alarm: " + cursor.getString(4) );
        //
        ...
        for(int i=2; i<6; i++){
            ...
            if (cursor.moveToNext()){
                ...
                Log.v("DB_day1_pm", " loaded med"+i+"-symbol is: " + cursor.getString(0) );
                ...
                Log.v("DB_day1_pm", "minute value for med" + i+ " is: " + Integer.parseInt(cursor.getString(3)));

                //
                pref.putBoolean("med"+i+"-alarm",Boolean.parseBoolean(cursor.getString(4) ));
                Log.v("DB_day1_pm", " Boolean value for loaded med"+i+"-alarm is: " + cursor.getString(4) );
                //
            }
...
//
    ...
    public void deleteAllRec()
    {
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DELETE FROM " +  TABLE_MED_1_PM); // + " WHERE " + COLUMN_NAME + " IN (SELECT " +  COLUMN_NAME + " FROM " +  TABLE_MED_1_PM + " ORDER BY " +  COLUMN_NAME + " ASC LIMIT 5)");
        db.close();
...
//        String query =  "DELETE FROM " + TABLE_MED_1_PM +  " WHERE " + COLUMN_SYMBOL + " IN (SELECT " +  COLUMN_SYMBOL + " FROM " +  TABLE_MED_1_PM + " LIMIT 5)";
//
//
//        SQLiteDatabase db = this.getWritableDatabase();
//
